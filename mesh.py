...
class Cuboid:
    def __init__(self, numberOfElements, numberOfNodes, nodesPerElement, dofPerNode, numberOfGaussPoints, fixedDofs, \
                 length, width, height, initValues,epotential0, alphaNM, betaNM, timeStep, \
                 lmu, llambda, gamma,beta1,beta2,a):
        ...
        for z in linspace(0, height, nodesPerDim):
            for y in linspace(0, width, nodesPerDim):
                for x in linspace(0, length, nodesPerDim):
                    self.nodes.append(Node(dofPerNode, [x, y, z, epotential0], initValues, alphaNM, betaNM, timeStep))
        ...
        for elementNumber in range(0, numberOfElements):
            self.elements.append(Element(nodesPerElement, dofPerNode, numberOfGaussPoints, mC, \
                                 lmu, llambda, gamma,beta1,beta2,a))
    ...
    def setExternalForces(self, externalForces):
    ...
    def computeDisplacements(self):
        ...
        displacements = solve(self.stiffness,self.forces)
...
#            outputFile.write("\n")
        ...
        outputFile.write("SCALARS\tphi\tfloat 1\n")
        outputFile.write("LOOKUP_TABLE\tdefault\n")
